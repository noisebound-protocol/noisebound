...
function createX402PQCHeader(params, keypair) {
  ...
  const signature = mlDsa.ml_dsa65.sign(keypair.signingKey, msgHash);
  const header = {
...
function verifyX402PQCHeader(header) {
  ...
  try {
    ...
    const ok = mlDsa.ml_dsa65.verify(publicKeyBytes, msgHash, signatureBytes);
...
function deriveAesKey(sharedSecret) {
  return hkdf.hkdf(sha2.sha256, sharedSecret, void 0, ENC_INFO, 32);
}
function encryptPaymentMetadata(metadata, payeePublicKey) {
  const { ciphertext: kemCiphertext, sharedSecret } = pqcWallet.encapsulateKey(payeePublicKey);
  const aesKey = deriveAesKey(sharedSecret);
  const aesNonce = randomBytes(12);
  const plaintext = new TextEncoder().encode(JSON.stringify(metadata));
  const aesCiphertext = aes.gcm(aesKey, aesNonce).encrypt(plaintext);
  return { kemCiphertext, aesCiphertext, aesNonce, payeePublicKey };
}
function decryptPaymentMetadata(encrypted, privateKey) {
  const sharedSecret = pqcWallet.decapsulateKey(encrypted.kemCiphertext, privateKey);
  const aesKey = deriveAesKey(sharedSecret);
  const plaintext = aes.gcm(aesKey, encrypted.aesNonce).decrypt(encrypted.aesCiphertext);
...
function createSession(payerKeypair, _payeeKEMPublicKey) {
  ...
  const signature = mlDsa.ml_dsa65.sign(payerKeypair.signingKey, msgToSign);
  return {
    sessionId,
...
function confirmSession(sessionOpen, payeeKeypair) {
  ...
  const { ciphertext, sharedSecret: _sharedSecret } = pqcWallet.encapsulateKey(payerKemPublicKey);
  ...
  const signature = mlDsa.ml_dsa65.sign(payeeKeypair.signingKey, msgToSign);
  return {
...
function deriveSessionKey(sessionOpen, confirmation, privateKey) {
  ...
  const sharedSecret = pqcWallet.decapsulateKey(kemCiphertext, privateKey);
  const salt = new TextEncoder().encode(sessionOpen.sessionId);
  return hkdf.hkdf(sha2.sha256, sharedSecret, salt, SESSION_INFO, 32);
}
...
function createSessionPayment(params, sessionKey, sessionId) {
  ...
  const mac = hmac.hmac(sha2.sha256, sessionKey, macInput);
  const payload = {
    s: sessionId,
    n: nonce,
    t: timestamp,
...
function verifySessionPayment(header, sessionKey) {
  ...
  try {
    ...
    const expected = hmac.hmac(sha2.sha256, sessionKey, macInput);
...
function issueCapabilityToken(granterKeypair, sessionId, executionAddress, scopes, ttlSeconds) {
  ...
  const signature = mlDsa.ml_dsa65.sign(granterKeypair.signingKey, msgHash);
  return {
    tokenId,
    sessionId,
    scopes,
...
function verifyCapabilitySignature(token) {
  try {
    ...
    return mlDsa.ml_dsa65.verify(publicKeyBytes, msgHash, signatureBytes);
  } catch {
    return false;
  }
}
...
async function executeScopedTransaction(token, executionSigner, tx, registry) {
  ...
  const activeNetwork = pqcWallet.getActiveNetwork();
...
exports.SIGNING_ALGORITHM = SIGNING_ALGORITHM;
...
exports.decryptPaymentMetadata = decryptPaymentMetadata;
exports.deriveSessionKey = deriveSessionKey;
exports.encryptPaymentMetadata = encryptPaymentMetadata;
exports.executeScopedTransaction = executeScopedTransaction;
exports.issueCapabilityToken = issueCapabilityToken;
exports.verifyCapabilitySignature = verifyCapabilitySignature;
exports.verifyCapabilityToken = verifyCapabilityToken;
exports.verifySessionPayment = verifySessionPayment;
