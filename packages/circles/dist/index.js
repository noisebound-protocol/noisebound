...
var OctraClient = class {
  ...
  async getNetworkInfo() {
    ...
    try {
      ...
      const detail = isAxiosError(err) ? `HTTP ${err.response?.status ?? "timeout"}: ${err.message}` : String(err);
...
var Circle = class {
  ...
  async execute(instruction, inputs) {
    const txHash = "0x" + toHex2(sha256(new TextEncoder().encode(`${this.address}:${instruction}:${Date.now()}`))).slice(0, 32);
...
function fhe_load_pk(pkBytes) {
  ...
  const keyId = toHex3(sha256(pkBytes)).slice(0, 16);
  const scale = 2 ** 40;
...
function signOctraTx(txBody, privKeyHex) {
  ...
  const pubSpki = createPublicKey(privKey).export({ type: "spki", format: "der" });
